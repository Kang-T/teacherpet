...
(() => {
  ...
  function decide(b) {
    ...
    add('walk', 0.9 * T.curiosity * (d.energy / 100) * (mode === 'class' ? 0 : 1), () => goTo(b, b.x + rand(-1, 1) * rand(3, 10) * (d.old ? 0.6 : 1) * (T.curiosity > 1.4 ? 1.6 : 1), 'walk'));
  ...
  function update(b, dt) {
    ...
    if (b.anim === 'chase') {
      ...
      if (dist > reach) {
        ...
        if (!worm.held && worm.y <= 0.05 && !worm.carrier) { grabWorm(b); return; }
