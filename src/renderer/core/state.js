...
(function (g) {
  ...
  function defaultState() {
    return {
      ...
      newsSeen: (TP.news && TP.news.latest()) || '',   // 처음 시작하는 아이는 '새 소식'을 이미 본 셈이다
      visitors: [],                         // 놀러 온 친구 닭 — 그날 하루 머문다 (flock 에 넣지 않는다)
  ...
  function migrate(saved) {
    ...
    if (!Array.isArray(out.visitors)) out.visitors = [];
    out.newsSeen = typeof s.newsSeen === 'string' ? s.newsSeen : '';   // 전에 쓰던 아이는 아직 새 소식을 못 봤다
